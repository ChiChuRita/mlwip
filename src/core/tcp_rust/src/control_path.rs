...
//! - `tcp_types`: Common types (TcpFlags, TcpSegment, validation enums)
//! - `tcp_api`: API functions (tcp_bind, tcp_listen, tcp_connect, etc.)
//! - `components/*`: Component-specific methods for state transitions
//! - `tcp_in`: Input dispatcher using component methods
//!
//! This module is kept temporarily for legacy test compatibility.
//! New code should use the modules listed above.

use crate::state::{TcpConnectionState, TcpState};
use crate::ffi;
use crate::tcp_proto;

// Re-export types (prefer using tcp_types directly)
pub use crate::tcp_types::{TcpFlags, TcpSegment, RstValidation, AckValidation, InputAction};
